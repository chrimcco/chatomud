  ...
  module Actions
    ...
    class SpawnItem < BaseAction
      ...
      def exec
        ...
        return unless check_target_present(item_template.inventory_controller, "Item template '#{item_template_code}' is not a container and cannot be outfitted.")

        item_outfitter = ItemOutfitter.find_by_code(item_outfitter_code)

        return unless check_target_present(item_outfitter, "Cannot find item outfitter with code '#{item_outfitter_code}'.")

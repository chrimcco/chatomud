  ...
  module Actions
    ...
    class GetContainer < BaseAction
      ...
      def exec
        ...
        source_controller = @character_controller.search_furniture_controller(source) unless source_controller

        return unless check_target_present(source_controller, "You cannot find the '#{source[:word]}'.")

        return unless check_is_container(source_controller)

        if check_is_closable(source_controller.inventory_controller, nil)
          return unless check_must_be_open(source_controller.inventory_controller.lid_controller, "#{source_controller.short_desc} is closed.")
        end
        ...
        if amount
          return unless check_is_stackable(target_item_controller)
          ...
          new_item_controller = handle_spawned_item(target_item_controller, amount, @character_controller.inventory_controller)

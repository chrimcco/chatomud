...
class Critical < ApplicationRecord
  ...
  def attack_stun_is_valid
    if no_attack_stun_applied?
      errors.add(:attack_stun_rounds,  "only usable when attack_stun_type is applicable") if attack_stun_rounds > 0
      errors.add(:attack_stun_penalty, "only usable when attack_stun_type is :attack_penalty") if attack_stun_penalty > 0
    elsif no_attack?
      errors.add(:attack_stun_rounds, "minimum 1 when attack_stun_type is :no_attack") if attack_stun_rounds < 1
      errors.add(:attack_stun_penalty, "not usable when attack_stun_type is :attack_penalty") if attack_stun_penalty > 0
  ...
  def parry_stun_is_valid
    if no_parry_stun_applied?
      errors.add(:parry_stun_rounds,  "only usable when parry_stun_type is applicable") if parry_stun_rounds > 0
      errors.add(:parry_stun_penalty, "only usable when parry_stun_type is :parry_penalty") if parry_stun_penalty > 0
    elsif no_parry?
      errors.add(:parry_stun_rounds, "minimum 1 when parry_stun_type is :no_parry") if parry_stun_rounds < 1
      errors.add(:parry_stun_penalty, "not usable when parry_stun_type is :parry_penalty") if parry_stun_penalty > 0
    elsif parry_penalty?

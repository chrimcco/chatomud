...
(1..(table.length - 1)).each do |height|
  ...
  (1..20).each do |against|
    ...
    puts "Attack.create!(score_limit: #{score}, skill: #{skill}, against: #{against}, hits: #{hits}, critical_severity: #{critical_severity}, critical_type: #{critical_type}, connects: #{connects})"
    if add_extra != nil
      puts "Attack.create!(score_limit: #{add_extra}, skill: #{skill}, against: #{against}, hits: 0, critical_severity: :no_critical, critical_type: :not_applicable, connects: false)"
    end    
  end
end
